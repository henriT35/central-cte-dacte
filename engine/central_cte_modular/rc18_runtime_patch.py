...
RC18_VERSION = "2.7.0 RC18 — Correção de Classificação Fiscal e Pedágio JSP"
...
def install_rc18_runtime(target_globals: MutableMapping[str, Any], bootstrap_state: Any) -> dict[str, Any]:
    namespace = bootstrap_state.compatibility_module
    # RC17 carrega estes blocos sob demanda. A RC18 os materializa antes de
    # trocar as funções, evitando que uma instalação tardia restaure o
    # classificador histórico depois do patch.
    lazy_registry = getattr(namespace, "CENTRAL_CTE_LEGACY_LAZY_REGISTRY", None)
    if lazy_registry is not None:
        lazy_registry.ensure("commercial_validation", trigger="rc18_startup")
        lazy_registry.ensure("report_excel", trigger="rc18_startup")
    else:
        install_commercial_validation_compat(namespace.__dict__)
        install_report_excel_compat(namespace.__dict__)
    original_validate = namespace.validate_cte_value
    ...
    orchestrator = getattr(namespace, "MODULAR_CTE_VALUE_ORCHESTRATOR", None)
    ...
    def validate_cte_value(info: dict[str, Any], base_data: Any, tables: Any):
        return _enrich_result(namespace, original_validate(info, base_data, tables), info, base_data, tables)

    namespace.validate_cte_value = validate_cte_value
    namespace.APP_VERSION = RC18_VERSION
    target_globals.update({
        "APP_VERSION": RC18_VERSION,
        ...
        "RC18_RUNTIME_PATCH": True,
    })
    return {
        "version": RC18_VERSION,
        "active": True,
...
__all__ = ["RC18_VERSION", "install_rc18_runtime"]
