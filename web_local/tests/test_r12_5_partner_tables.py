...
def test_release_version_is_r12_5() -> None:
    ...
    assert "MVP13 R12.5" in server
    assert "MVP13 R12.5" in app_js
    assert "central-cte-dacte:r12.5" in compose
...
def test_compiled_table_contains_both_partners_and_expected_counts() -> None:
    ...
    assert len(tables["partners"]) == 15
    assert len(tables["rules"]) == 278
    assert len(tables["regions"]) == 379
    assert len(tables["extras"]) == 102
    assert "OPAL" in tables["partners"]
    assert "AC_LOG_C_VARGAS" in tables["partners"]
    assert tables["partners"]["AC_LOG_C_VARGAS"].get("cnpj", "") == ""
...
def test_ac_log_exact_routes_keep_percent_minimum_ton_rate_and_gris() -> None:
    tables = PartnerTableRepository().load(COMPILED)
    engine = _commercial_engine()
